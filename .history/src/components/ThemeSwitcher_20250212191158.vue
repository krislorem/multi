...
const switchTheme = async (event: MouseEvent) => {
  ...
  mask.style.cssText = `
    ...
    width: 100vw;
    height: 100vh;
    background: ${newTheme === 'light' ? '#fff' : '#000 '};
    clip-path: ${newTheme === 'light' ? `circle(0% at ${x}px ${y}px)` : `circle(150% at ${x}px ${y}px)`};
    z-index: 9999;
    pointer-events: none;
  ...
  await gsap.to(mask, {
    clipPath: newTheme === 'light' ? `circle(150% at ${x}px ${y}px)` : `circle(0% at ${x}px ${y}px)`,
    duration: 0.5,
    ease: 'power4.out'
  })
