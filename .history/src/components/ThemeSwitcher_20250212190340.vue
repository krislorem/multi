...
const switchTheme = async (event: MouseEvent) => {
  ...
  const newTheme = targetTheme.value === 'dark' ? 'dark' : 'light'
  ...
  mask.style.cssText = `
    ...
    clip-path: ${newTheme === 'dark' ? `circle(0% at ${x}px ${y}px)` : `circle(150% at ${x}px ${y}px)`};
    z-index: 9999;
    pointer-events: none;
  ...
  await gsap.to(mask, {
    clipPath: newTheme === 'dark' ? `circle(150% at ${x}px ${y}px)` : `circle(0% at ${x}px ${y})`,
    duration: 0.5,
    ease: 'power4.out'
  })
...
<template>
  <button @click="switchTheme" class="theme-switcher"
    :title="`Switch to ${targetTheme === 'light' ? 'dark' : 'light'} theme`">
    <transition name="fade" mode="out-in">
      <span :key="targetTheme">
        {{ targetTheme === 'light' ? '🌙' : '☀️' }}
      </span>
    </transition>
  </button>
