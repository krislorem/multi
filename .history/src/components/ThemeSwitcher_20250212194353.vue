...
const switchTheme = async (event: MouseEvent) => {
  ...
  mask.style.cssText = `
    ...
    width: 100vw;
    height: 100vh;
    background: ${targetTheme === 'dark' ? `#fff` : `#000`};
    clip-path: ${targetTheme === 'dark' ? `circle(150% at ${x}px ${y}px)` : `circle(150% at ${x}px ${y}px)`};
    z-index: 9999;
    pointer-events: none;
  ...
  await gsap.to(mask, {
    clipPath: targetTheme === 'dark' ? `circle(0% at ${x}px ${y}px)` : `circle(0% at ${x}px ${y}px)`,
    duration: 3,
    ease: 'power4.out'
  })
