...
const switchTheme = async (event: MouseEvent) => {
  ...
  mask.style.cssText = `
    ...
    height: 100vh;
    background: '#000';
    clip-path: ${targetTheme.value === 'light' ? `circle(150% at ${x}px ${y}px)` : `circle(0% at ${x}px ${y}px)`};
    z-index: 9999;
    pointer-events: none;
  ...
  await gsap.to(mask, {
    clipPath: targetTheme.value === 'light' ? `circle(0% at ${x}px ${y}px)` : `circle(150% at ${x}px ${y}px)`,
    duration: 0.5,
    ease: 'power4.out'
  })
